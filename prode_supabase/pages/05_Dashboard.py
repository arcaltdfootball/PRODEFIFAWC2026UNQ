...
# ── Estado de boletas: quién no tiene 6/6 pronósticos ────────────────────────
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("### 📋 Estado de boletas")

# Todos los partidos del torneo (sin importar si tienen resultado)
todos_partido_ids = [p["id"] for p in resp_part2.data]
total_partidos_torneo = len(todos_partido_ids)  # debería ser 72
...
prons_por_part = {}
for pr in resp_pron_todos.data:
    pid = pr["participante_id"]
    prons_por_part.setdefault(pid, 0)
    prons_por_part[pid] += 1
...
for p in resp_part.data:
    p_id      = p["id"]
    p_nom     = p["nombre"]
    cargados  = prons_por_part.get(p_id, 0)
...
incompletos.sort(key=lambda x: -x[2])  # ordenar por más faltantes primero
