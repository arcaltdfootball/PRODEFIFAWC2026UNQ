...
# Dieciseisavos NO vive en la tabla "partidos": los cruces están en la tabla
# "dieciseisavos" y sus pronósticos en "pronosticos_dieciseisavos", con la
# columna "cruce_id" (no "partido_id"). select("*") evita romper si falta
# alguna columna (ej. "resultado" todavía sin cargar).
resp_cruces_16 = sb.table("dieciseisavos").select("*").order("partido_num").execute()
cruces_16          = resp_cruces_16.data
total_partidos_16  = len(cruces_16)

if total_partidos_16 == 0:
    st.info("Todavía no se cargaron los cruces de Dieciseisavos de Final.")
else:
    def _pronostico_valido(valor):
        # Un pronóstico cuenta como "cargado" solo si tiene marcador, ej. "2-1"
        return bool(valor) and "-" in str(valor)

    # Traer TODOS los pronósticos de dieciseisavos, paginando por si son +1000 filas
    pron_16_rows = []
    page_size = 1000
    start = 0
    while True:
        resp_page = (
            ...
            .select("participante_id, cruce_id, pronostico")
            .range(start, start + page_size - 1)
            .execute()
        )
        pron_16_rows.extend(resp_page.data)
        if len(resp_page.data) < page_size:
            break
        start += page_size

    pron_16_validos       = [r for r in pron_16_rows if _pronostico_valido(r.get("pronostico"))]
    total_pronosticos_16  = len(pron_16_validos)

    # Aciertos: solo sobre cruces que ya tienen resultado cargado
    resultados_map_16       = {c["id"]: c.get("resultado") for c in cruces_16 if c.get("resultado")}
    partidos_jugados_16_ids = list(resultados_map_16.keys())
    resueltos_16             = len(partidos_jugados_16_ids)

    pron_comparables_16 = sum(1 for r in pron_16_validos if r["cruce_id"] in resultados_map_16)
    aciertos_16 = sum(
        1 for r in pron_16_validos
        if r["cruce_id"] in resultados_map_16
        and resultados_map_16[r["cruce_id"]] == r["pronostico"]
    )
    ...
    col2_16.metric("Cruces jugados",      f"{resueltos_16} / {total_partidos_16}")
    ...
    # Cruces distintos con pronóstico válido, por participante (evita duplicados)
    cruces_cargados_por_participante_16 = {}
    for r in pron_16_validos:
        pid = r["participante_id"]
        cruces_cargados_por_participante_16.setdefault(pid, set()).add(r["cruce_id"])
    ...
    for p in resp_part.data:
        p_id  = p["id"]
        p_nom = p["nombre"]
        cargados_16  = len(cruces_cargados_por_participante_16.get(p_id, set()))
