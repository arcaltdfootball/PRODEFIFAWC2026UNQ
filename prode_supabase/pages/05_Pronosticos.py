...
# El pronóstico se guarda como marcador "gl-gv" (ej. "2-1"), no como "1"/"X"/"2".
# Lo parseamos acá para clasificar cada apuesta y además guardamos el marcador
# textual para mostrarlo debajo del nombre de cada participante.
...
votos_X  = {}
votos_2  = {}
marcador_de = {}  # uid -> "gl-gv" textual, para mostrar debajo del nombre

for uid, v in apuestas.items():
    ...
    if signo == "1":
        votos_1[uid] = nombre_part
    elif signo == "X":
        votos_X[uid] = nombre_part
    else:
        votos_2[uid] = nombre_part

sin_voto = {
    p2["id"]: p2["nombre"]
    for p2 in participantes_raw
    if signo_de_marcador(apuestas.get(str(p2["id"]))) is None
}
