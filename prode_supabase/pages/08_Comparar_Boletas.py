...
if st.session_state.get("cmp_activo"):
    ...
    # ── Resumen general de coincidencias ──────────────────────────────────────
    coinciden_exacto_n = 0
    coinciden_resultado_n = 0
    difieren_n = 0
    sin_comparar_n = 0
    ...
    for p in partidos_raw:
        ...
        if r1 is None or r2 is None:
            ...
            # mismo 1/X/2 — ¿también coincide el marcador?
    ...
    st.markdown(
        ...
        f'<div class="num" style="font-family:\'Bebas Neue\',sans-serif;font-size:2rem;line-height:1;color:#e8c96b;">{coinciden_exacto_n}</div>'
        ...
        f'<div class="num" style="font-family:\'Bebas Neue\',sans-serif;font-size:2rem;line-height:1;color:#4ade80;">{coinciden_resultado_n}</div>'
        ...
        f'<div class="num" style="font-family:\'Bebas Neue\',sans-serif;font-size:2rem;line-height:1;color:#f87171;">{difieren_n}</div>'
        ...
        f'<div class="num" style="font-family:\'Bebas Neue\',sans-serif;font-size:2rem;line-height:1;color:#94a3b8;">{sin_comparar_n}</div>'
    ...
    for p in lista_part_grupo:
        ...
        v1 = pron1["resultado"] if pron1 else None
        v2 = pron2["resultado"] if pron2 else None
        gl1 = pron1["goles_local"]     if pron1 else None
        gv1 = pron1["goles_visitante"] if pron1 else None
        gl2 = pron2["goles_local"]     if pron2 else None
        gv2 = pron2["goles_visitante"] if pron2 else None
        ...
        if v1 is not None and v2 is not None and v1 == v2:
            # Mismo 1/X/2 — verificar si además coincide el marcador
            marcador_igual = (
        ...
        def pill_html(valor, gl, gv):
            if gl is not None and gv is not None:
                marcador = (
                    ...
                    f'color:#e8c96b;line-height:1;margin-bottom:2px;">'
                    f'{int(gl)} - {int(gv)}</div>'
                )
            else:
                marcador = ""
            if valor == "1":
                return marcador + f'<div class="comp-pill op1">1</div><div class="comp-sub">Gana {local}</div>'
            if valor == "X":
                return marcador + '<div class="comp-pill opX">X</div><div class="comp-sub">Empate</div>'
            if valor == "2":
                return marcador + f'<div class="comp-pill op2">2</div><div class="comp-sub">Gana {visitante}</div>'
        ...
        card_html = (
            ...
            + '<div class="comp-wrap">'
            + '<div class="comp-col">'
            + f'<div class="comp-col-nombre">{n1_sel}</div>'
            + pill_html(v1, gl1, gv1)
            + "</div>"
            + '<div class="comp-col">'
            + f'<div class="comp-col-nombre">{n2_sel}</div>'
            + pill_html(v2, gl2, gv2)
            + "</div>"
            + "</div>"
            + "</div>"
        )
