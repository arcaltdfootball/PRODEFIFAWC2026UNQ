...
st.markdown(
    ...
    /* DOTS DE NAVEGACIÓN (igual a 01_Resultados.py) */
    .dots-row-clickable {
...
# ══════════════════════════════════════════════════════════════════════════════
ADMIN_PASSWORD = st.secrets.get("ADMIN_PASSWORD", "aleotero")  # definí ADMIN_PASSWORD en secrets.toml
...
for numero, p in enumerate(participantes_filtrados, start=1):
    ...
    if col_edit is not None:
        with col_edit:
            ...
            if st.button(label_edit, key="edit_part_" + str(p_id), use_container_width=True,
                         help="Editar nombre/foto de " + p_nom):
    ...
    # ── Panel edición nombre/foto (solo admin) ─────────────────────────────────
    ...
    if expandido:
        ...
        if not todos_partidos:
            ...
            goles_pron_dict = {r["partido_id"]: (r.get("goles_local"), r.get("goles_visitante")) for r in resp_pron.data}
            ...
            # ── Dots de navegación (mismo estilo visual que 01_Resultados.py) ───
            dots_inner = ""
            ...
            st.markdown(
                '<div class="dots-row-clickable">' + dots_inner + '</div>',
                unsafe_allow_html=True
            )
            ...
            goles_pron_local, goles_pron_visit = goles_pron_dict.get(partido_id, (None, None))

            img_local = get_flag_img(local,  size=52)
            img_visit = get_flag_img(visitante, size=52)

            # Marcador pronosticado
            if goles_pron_local is not None and goles_pron_visit is not None:
                marcador_pron_html = (
                    ...
                    'color:#e8c96b;line-height:1;">' + str(int(goles_pron_local)) + '</span>'
                    '<span style="font-family:\'Bebas Neue\',sans-serif;font-size:1.6rem;'
                    'color:#475569;">-</span>'
                    '<span style="font-family:\'Bebas Neue\',sans-serif;font-size:2.4rem;'
                    'color:#e8c96b;line-height:1;">' + str(int(goles_pron_visit)) + '</span>'
                    '</div>'
                )
            else:
                marcador_pron_html = (
                    '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1.6rem;'
                    'color:#475569;text-align:center;margin-bottom:8px;">VS</div>'
                )
            ...
            if es_admin:
                ...
                st.markdown(
                    ...
                    '⚽ Marcador pronosticado</div>',
                    unsafe_allow_html=True
                )
                col_gl, col_sep2, col_gv, col_save_score = st.columns([2, 0.5, 2, 2])
                with col_gl:
                    gl_input = st.number_input(
                        local[:10], min_value=0, max_value=30,
                        value=int(goles_pron_local) if goles_pron_local is not None else 0,
                        key="gl_pron_" + str(p_id) + "_" + str(partido_id),
                        label_visibility="collapsed"
                    )
                with col_sep2:
                    st.markdown('<div style="text-align:center;padding-top:8px;'
                                'font-weight:700;color:#94a3b8;font-size:18px;">-</div>',
                                unsafe_allow_html=True)
                with col_gv:
                    gv_input = st.number_input(
                        visitante[:10], min_value=0, max_value=30,
                        value=int(goles_pron_visit) if goles_pron_visit is not None else 0,
                        key="gv_pron_" + str(p_id) + "_" + str(partido_id),
                        label_visibility="collapsed"
                    )
                with col_save_score:
                    if st.button("⚽ Guardar marcador", key="save_score_" + str(p_id) + "_" + str(partido_id),
                                 use_container_width=True):
                        # Derivar 1/X/2 del marcador
                        if gl_input > gv_input:
                            res_marcador = "1"
                        elif gl_input < gv_input:
                            res_marcador = "2"
                        else:
                            res_marcador = "X"
                        existente = (
                        ...
                        if existente.data:
                            ...
                            sb.table("pronosticos").update({
                                "pronostico":      res_marcador,
                                "goles_local":     gl_input,
                                "goles_visitante": gv_input,
                        ...
                        st.toast(f"Marcador {gl_input}-{gv_input} guardado.", icon="⚽")
                        st.rerun()
