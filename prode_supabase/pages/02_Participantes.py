...
def obtener_acceso_bloqueado() -> bool:
    ...
    try:
        ...
        sb.table("configuracion_app").insert({"id": 1, "acceso_bloqueado": False}).execute()
        return False
    except Exception:
...
def set_acceso_bloqueado(valor: bool) -> bool:
    ...
    try:
        existente = sb.table("configuracion_app").select("id").eq("id", 1).execute()
        if existente.data:
            sb.table("configuracion_app").update({"acceso_bloqueado": valor}).eq("id", 1).execute()
        else:
            sb.table("configuracion_app").insert({"id": 1, "acceso_bloqueado": valor}).execute()
        return True
    except Exception as ex:
        st.error(
            "No se pudo actualizar el estado de acceso. ¿Existe la tabla "
            "'configuracion_app' en Supabase? Detalle: " + str(ex)
        )
        return False
...
if acceso_bloqueado and not es_admin:
    st.markdown(
        '<div style="background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.4);'
        'border-radius:10px;padding:10px 16px;text-align:center;margin-bottom:10px;">'
        '<span style="color:#f87171;font-weight:700;font-size:13px;">'
        '🔒 El acceso de participantes está bloqueado temporalmente por el administrador.</span>'
        '</div>',
        unsafe_allow_html=True
    )
