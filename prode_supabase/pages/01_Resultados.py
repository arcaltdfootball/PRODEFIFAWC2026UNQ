...
resp = sb.table("partidos").select("id, grupo, fecha, hora, sede, local, visitante, resultado, goles_local, goles_visitante").order("grupo").order("fecha").order("hora").execute()
...
p_id, grupo_val, fecha, hora, sede, local, visitante, resultado, goles_local, goles_visitante = lista_partidos[idx]
...
if goles_local is not None and goles_visitante is not None:
    score_display = (
        '<div class="score-block" style="display:flex;flex-direction:row;align-items:center;gap:4px;min-width:80px;justify-content:center;">'
        '<span class="score-number" style="font-size:2.6rem;color:#e8c96b;">' + str(int(goles_local)) + '</span>'
        '<span style="font-family:\'Bebas Neue\',sans-serif;font-size:1.8rem;color:#475569;">-</span>'
        '<span class="score-number" style="font-size:2.6rem;color:#e8c96b;">' + str(int(goles_visitante)) + '</span>'
        '</div>'
    )
else:
    score_display = '<div class="score-block"><span class="score-number">VS</span></div>'
