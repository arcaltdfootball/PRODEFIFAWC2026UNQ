...
for i, row in enumerate(ranking):
    ...
    try:
        efectividad = round((float(aciertos) / float(disputados)) * 100, 1) if disputados else None
        if efectividad is not None:
            if math.isnan(efectividad) or math.isinf(efectividad):
                efectividad = None
            else:
...
if df.empty:
    ...
    def build_card(row):
        ...
        # Desglose Grupos (solo si tiene datos)
        ...
        # Desglose Dieciseisavos (solo si tiene partidos disputados)
