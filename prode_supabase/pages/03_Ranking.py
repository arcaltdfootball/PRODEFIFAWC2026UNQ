...
# ── Cargar posiciones anteriores desde Supabase ───────────────────────────────
from supabase_client import get_supabase
sb = get_supabase()

def cargar_posiciones_anteriores():
    try:
        resp = sb.table("ranking_snapshot").select("nombre,posicion").execute()
        return {r["nombre"]: r["posicion"] for r in (resp.data or [])}
    except Exception:
        return {}

def guardar_posiciones_actuales(nuevas: dict):
    try:
        rows = [{"nombre": k, "posicion": v} for k, v in nuevas.items()]
        sb.table("ranking_snapshot").upsert(rows, on_conflict="nombre").execute()
    except Exception:
        pass

posiciones_anteriores = cargar_posiciones_anteriores()
nuevas_posiciones = {}
datos = []
...
guardar_posiciones_actuales(nuevas_posiciones)
