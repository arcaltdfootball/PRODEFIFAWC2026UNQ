...
def calcular_todas_las_posiciones() -> dict:
    ...
    if not equipos:
        raise RuntimeError(
            "La tabla 'equipos' no devolvió filas. Revisá que tenga datos "
            "cargados (importar equipos_clausura_2026.csv) y que, si tiene "
            "Row Level Security activado, exista una policy de SELECT para "
            "el rol 'anon' (o el rol que use tu conexión)."
        )
    ...
    for p in partidos:
        ...
        if local not in stats or visitante not in stats:
            ...
            if local not in stats:
                nombres_sin_match.add(local)
            if visitante not in stats:
                nombres_sin_match.add(visitante)
            continue
