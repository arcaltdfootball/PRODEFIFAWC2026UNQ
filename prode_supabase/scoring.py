...
def _calcular_fase(sb, tabla_pron: str, tabla_part: str, id_col_pron: str, id_col_part: str, participante_id: int):
    ...
    resp_part = (
        sb.table(tabla_part)
        .select(id_col_part + ", resultado, goles_local, goles_visitante, marcador")
    ...
    for partido in resp_part.data:
        ...
        if not resultado_oficial:
            # Intentar derivarlo del marcador oficial si existe
            marcador_oficial = partido.get("marcador") or ""
            if not marcador_oficial:
                gl_of = partido.get("goles_local")
                gv_of = partido.get("goles_visitante")
                if gl_of is not None and gv_of is not None:
                    marcador_oficial = str(gl_of) + "-" + str(gv_of)
            resultado_oficial = _resultado_desde_marcador(marcador_oficial)
        ...
        # También intentar desde columna marcador si goles_local/visitante no están
        if gl_oficial is None or gv_oficial is None:
            marc_of = partido.get("marcador") or ""
            if marc_of and "-" in marc_of:
                try:
                    parts = marc_of.split("-")
                    gl_oficial = int(parts[0])
                    gv_oficial = int(parts[1])
                except (ValueError, IndexError):
                    pass
