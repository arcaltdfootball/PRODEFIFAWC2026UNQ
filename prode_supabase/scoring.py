...
def _calcular_fase(sb, tabla_pron: str, tabla_part: str, id_col_pron: str, id_col_part: str, participante_id: int):
    ...
    if tabla_part == "partidos":
        ...
        resp_part = (
            sb.table(tabla_part)
            .select(id_col_part + ", resultado, marcador")
            .in_(id_col_part, ids_pronosticados)
            .execute()
        )
    ...
    for partido in resp_part.data:
        ...
        if tabla_part == "partidos":
            ...
            gl_oficial = gv_oficial = None
            marcador = partido.get("marcador") or ""
            if "-" in marcador:
                try:
                    gl_oficial, gv_oficial = map(int, marcador.split("-"))
                except (ValueError, IndexError):
                    pass
