...
def render_messiboot():
    ...
    #messiboot-bubble {{
        width: 58px; height: 58px; border-radius: 50%; margin: 0 16px 16px 0;
    ...
    #messiboot-panel {{
        width: 340px; max-height: 460px; margin: 0 16px 12px 0;
    ...
    <div id="messiboot-wrap">
        <div id="messiboot-panel" class="{'open' if pregunta_pendiente else ''}">
            ...
            <form id="messiboot-form" target="_parent" method="GET">
                <div id="messiboot-inputrow">
                    <input id="messiboot-input" type="text" placeholder="Preguntame sobre el prode..." autocomplete="off">
                    <button id="messiboot-send" type="submit">➤</button>
                </div>
            </form>
        </div>
    ...
    function messibootApplyOpenState() {{
        ...
        messibootResizeIframe();
    }}

    function messibootResizeIframe() {{
        try {{
            ...
            if (messibootAbierto) {{
                ...
                // Cerrado: el iframe achica a solo el tamaño de la burbuja,
                // así no tapa ni bloquea clics en el resto de la página.
    ...
    // El propio iframe que Streamlit genera para este componente necesita
    // estar posicionado fixed en la esquina (con fondo transparente) para
    // que la burbuja y el panel se vean flotando sobre TODO el sitio.
    // El tamaño (chico cuando está cerrado, grande cuando está abierto)
    // lo controla messibootResizeIframe(), llamada desde messibootApplyOpenState().
    (function messibootFixIframe() {{
        try {{
            const iframe = window.frameElement;
            if (!iframe) return;
        ...
        }} catch (e) {{ /* cross-origin: no debería pasar en mismo dominio */ }}
    }})();

    // El <body> del propio iframe debe quedar transparente y sin scroll
    // para que solo se vean la burbuja y el panel, nunca un rectángulo gris.
    document.documentElement.style.background = 'transparent';
    document.body.style.background = 'transparent';
    document.body.style.margin = '0';
    document.body.style.overflow = 'hidden';

    function messibootSend(ev) {{
        if (ev) ev.preventDefault();
        const input = document.getElementById('messiboot-input');
        const btn = document.getElementById('messiboot-send');
        const form = document.getElementById('messiboot-form');
        const texto = input.value.trim();
        if (!texto) return false;
        input.disabled = true;
        btn.disabled = true;

        // document.referrer SÍ está disponible dentro del iframe de un
        // componente de Streamlit (a diferencia de window.parent.location,
        // que el navegador bloquea por política cross-origin). La usamos
        // como base para armar la URL real de la app y navegar ahí con
        // target="_parent", que el navegador SÍ permite atravesar.
        let base = document.referrer || '';
        if (!base) {{
            try {{ base = window.top.location.href; }} catch (e) {{ base = ''; }}
        }}
        if (!base) {{
            alert('No se pudo determinar la página actual. Recargá el sitio e intentá de nuevo.');
            input.disabled = false;
            btn.disabled = false;
            return false;
        }}

        // Action = URL completa del padre, conservando sus query params
        // existentes y agregando/reemplazando messiboot_msg. Como el input
        // ya no tiene "name", el form no aporta su propio parámetro —
        // así evitamos que se duplique o se pierdan otros params del sitio.
    ...
    document.getElementById('messiboot-form').addEventListener('submit', messibootSend);
    ...
    # height/width iniciales: el script interno los ajusta dinámicamente
    # (90x90 cerrado, 380x560 abierto) vía window.frameElement, pero les
    # damos un valor de partida no-cero para evitar un parpadeo de 0px
    # mientras el script corre.
    altura_inicial = 560 if pregunta_pendiente else 90
    ancho_inicial = 380 if pregunta_pendiente else 90
    components.html(widget_html, height=altura_inicial, width=ancho_inicial)
